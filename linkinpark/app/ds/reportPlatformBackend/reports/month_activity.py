...
def month_activity(
    ...
    result = pd.DataFrame(
        pd.Series(pd.date_range(start, end - relativedelta(days=1), freq="d")))
    result = result.rename(columns={0: "content"})
    ...
    worksheet.title = "行事曆活動月報表"
    with pd.ExcelWriter(report_file, engine="openpyxl") as writer:
        writer.book = workbook
        writer.sheets = \
            dict((ws.title, ws) for ws in workbook.worksheets)
        result.to_excel(writer, sheet_name="行事曆活動月報表", index=False)
